...
data1 <- read.spss(file = "Public Voice - Genetics Society 311022.sav", to.data.frame=TRUE)
...
#attitude to genetics before pandemic 
## VERY NEGATIVE ARE 1, all others are 0 
#recoded so all negative attitudes to science are 1

data1$BeforePand_1 <- as.numeric(as.character(revalue(data1$BeforePand_1, c("Strongly agree"=1, "Agree"=0, "Disagree"=0,"Strongly disagree"=0, "Neither agree or disagree"=0 ))))
data1$BeforePand_2 <- as.numeric(as.character(revalue(data1$BeforePand_2, c("Strongly agree"=1, "Agree"=0, "Disagree"=0,"Strongly disagree"=0, "Neither agree or disagree"=0 ))))
data1$BeforePand_3 <- as.numeric(as.character(revalue(data1$BeforePand_3, c("Strongly agree"=0, "Agree"=0, "Disagree"=0,"Strongly disagree"=1, "Neither agree or disagree"=0 ))))


#each participant is given a score range -1 to +1 for attitude to genetics prior to pandemic

data1$attitude_prior <- rowSums( data1[,11:13] )/6.0i
...
#who to trust is scored 1 yes, 0 no. "None of these" is treated opposite, as is "Dont know"
...
#which to trust is scored 1 yes, 0 no. "None of these" is treated opposite, as is "Dont know"
...
data1$CovVac_recoded  <- as.numeric(as.character(revalue(data1$CovVac, c("Yes, and I have already been vaccinated"=0, "Yes, but I am yet to be vaccinated"=0, "No, I would not get vaccinated"=1, "Prefer not to answer"="NA" ))))
...
#recode politics - more positive more right wing

data1$RS_RichPeopleGetAwayWithIt <-as.numeric(as.character(revalue(data1$RS_RichPeopleGetAwayWithIt, c("Strongly agree"=-2, "Tend to agree"=-1, "Tend to disagree"=1,"Strongly disagree"=2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA ))))
data1$RS_RespectUKValues <-as.numeric(as.character(revalue(data1$RS_RespectUKValues, c("Strongly agree"=2, "Tend to agree"=1, "Tend to disagree"=-1,"Strongly disagree"=-2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA ))))
data1$RS_ProtectWorkingConditionsWages <-as.numeric(as.character(revalue(data1$RS_ProtectWorkingConditionsWages, c("Strongly agree"=-2, "Tend to agree"=-1, "Tend to disagree"=1,"Strongly disagree"=2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA ))))
data1$RS_BusinessOverregulated<-as.numeric(as.character(revalue(data1$RS_BusinessOverregulated, c("Strongly agree"=2, "Tend to agree"=1, "Tend to disagree"=-1,"Strongly disagree"=-2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA ))))
data1$RS_NationalisePublicServicesIndustries <-as.numeric(as.character(revalue(data1$RS_NationalisePublicServicesIndustries, c("Strongly agree"=-2, "Tend to agree"=-1, "Tend to disagree"=1,"Strongly disagree"=2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA ))))
data1$RS_TolerateUnconventional <-as.numeric(as.character(revalue(data1$RS_TolerateUnconventional, c("Strongly agree"=-2, "Tend to agree"=-1, "Tend to disagree"=1,"Strongly disagree"=2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA ))))
data1$RS_TougherSentences <-as.numeric(as.character(revalue(data1$RS_TougherSentences, c("Strongly agree"=2, "Tend to agree"=1, "Tend to disagree"=-1,"Strongly disagree"=-2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA ))))
data1$RS_RedistributeIncome <-as.numeric(as.character(revalue(data1$RS_RedistributeIncome, c("Strongly agree"=-2, "Tend to agree"=-1, "Tend to disagree"=1,"Strongly disagree"=2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA ))))
data1$RS_FewerImmigrants <-as.numeric(as.character(revalue(data1$RS_FewerImmigrants, c("Strongly agree"=2, "Tend to agree"=1, "Tend to disagree"=-1,"Strongly disagree"=-2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA))))
data1$RS_AbolishMonarchy <-as.numeric(as.character(revalue(data1$RS_AbolishMonarchy, c("Strongly agree"=-2, "Tend to agree"=-1, "Tend to disagree"=1,"Strongly disagree"=2, "Neither agree nor disagree"=0, "Dont know" = "NA", "NA"=NA))))
data1$politics <- rowMeans( data1[,99:108], na.rm=TRUE)/2.0

#recode sex - where "Identify as other" = "NA"
data1$RS_Sex <- revalue(data1$RS_Sex, c("Female"="Female", "Male"="Male", "Identify in another way"="NA"))

#recode ethnicity- where white british and everything else is other
data1$RS_EthnicGroup <- as.character(revalue(data1$RS_EthnicGroup, c("White British"="White British", "Any other White background"="Other", "Mixed/Multiple ethnic groups"="Other", "Indian"="Other", "Pakistani"="Other", "Any other single ethnic group"="Other", "Any other Asian background"="Other", "Black Caribbean"="Other", "Black African"= "Other", "Arab"="Other", "Chinese"="Other", "Bangladeshi"="Other", "Any other Black background"= "Other" )))
write.csv(data1, "survey_data_recoded_mine_veryneg.csv", row.names = FALSE)
